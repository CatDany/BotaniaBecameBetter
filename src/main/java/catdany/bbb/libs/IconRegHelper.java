...
public class IconRegHelper
{
	...
	{
		return reg.registerIcon(Refs.RESLOC + ":" + item.getUnlocalizedName().substring(5 + Refs.MODID.length() + 1) + suffix);
	}
	
	public static IIcon regBlock(Block block, IIconRegister reg, String suffix)
	{
		return reg.registerIcon(Refs.RESLOC + ":" + block.getUnlocalizedName().substring(5 + Refs.MODID.length() + 1) + suffix);
	}
}